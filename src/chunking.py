    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for m in merged:
            if len(m) > self.chunk_size:
                if next_seps:
                    final_chunks.extend(self._split(m, next_seps))
                else:
                    final_chunks.extend([m[i:i+self.chunk_size] for i in range(0, len(m), self.chunk_size)])
            else:
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        return {
            "fixed_size": _stats(fc.chunk(text)),
            "by_sentences": _stats(sc.chunk(text)),
            "recursive": _stats(rc.chunk(text)),
        }
